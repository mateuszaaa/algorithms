...
UIOpenCV::UIOpenCV(const std::string& name): name_(name), mat_(500,500, CV_8UC3, {255,255,255}){
  Update();
}
...
void UIOpenCV::Plot(Edge e, Color c, Thickness t){
  ...
  std::tie(x1,y1) = from;
  std::tie(x2,y2) = to;
  cv::line(mat_, {x1,y1}, {x2,y2}, ToCvColor(c), ToUnderlying(t));
}
...
void UIOpenCV::Plot(Vertex v, Color c, Thickness t){
  int x,y;
  std::tie(x,y) = v;
  cv::circle(mat_, {x,y}, 5, ToCvColor(c) ,ToUnderlying(t));
}
